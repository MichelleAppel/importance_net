...
class DiscriminatorLoss():
    ...
    def __init__(self, model, gan_mode='lsgan'):
        '''
        Parameters:
            model (BaseModel): a cycleGAN model that contains discriminators for the two domains
            ganmode (String): the type of GAN objective. [vanilla | lsgan | wgangp]. vanilla GAN loss is the cross-entropy objective used in the original GAN paper.
        '''
        self.model = model
        self.loss_function = weighted_GANLoss(gan_mode).to(model.device)
    
    def criterion(self, importance_model):
        '''
        Parameters:
            importance_model (BaseModel): a model that weights the importance of each sample in the batch

        Returns:
            loss_W (Tensor): the loss of the importance model
        '''
        ...
        self.loss_D_real = self.loss_function(pred_real, True, importance_model.w)
